...
Bitset.prototype.toString = function() {
	if (!this.string) {
		...
		for (var i=0, ii=r.length-1; i<ii; i++) {
			if (r[0] == 0) { r.shift(); }
			else { break; }
		}
		this.string = r.join("");
...
Bitset.mutatingZero = function(a) {
...
Bitset.prototype.mutatingZero = function() {
	Bitset.mutatingZero(this);
}
...
Bitset.allIn = function(a, b) {
	...
	for (var i=0; i<n; i++) {
		var aan = (aa.length < i-1) ? 0 : aa[i];
		var bbn = (bb.length < i-1) ? 0 : bb[i];
		console.log('AND ' + aan.toString(2) + ' ' + bbn.toString(2) + ' ' + (aan & bbn).toString(2));	
...
Bitset.anyIn = function(a, b) {
	...
	for (var i=0; i<n; i++) {
		var aan = (aa.length < i-1) ? 0 : aa[i];
		var bbn = (bb.length < i-1) ? 0 : bb[i];
		console.log('OR ' + aan.toString(2) + ' ' + bbn.toString(2) + ' ' + (aan & bbn).toString(2));
